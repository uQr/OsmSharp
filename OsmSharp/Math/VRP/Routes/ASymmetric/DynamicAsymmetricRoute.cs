...
namespace OsmSharp.Math.VRP.Routes.ASymmetric
{
    ...
    public class DynamicAsymmetricRoute : IASymmetricRoute
    {
        ...
        private DynamicAsymmetricRoute(int first, int[] nextArray, bool isRound)
        {
            _first = first;
        ...
        /// <param name="isRound"></param>
        public DynamicAsymmetricRoute(int size, bool isRound)
        {
            _isRound = isRound;
        ...
        /// <param name="isRound"></param>
        public DynamicAsymmetricRoute(int size, int customer, bool isRound)
        {
            _isRound = isRound;
            _nextArray = new int[size];
            for (int idx = 0; idx < size; idx++)
        ...
        public bool Contains(int from, int to)
        {
            ...
            { // customers should exist.
                ...
                else if (this.Contains(from) && _nextArray[from] == -1)
        ...
        public bool IsValid()
        {
            HashSet<int> unique_customers = new HashSet<int>(_nextArray);
            unique_customers.Remove(-1);
            int count = 0;
            foreach (int customer in _nextArray)
            {
                if (customer >= 0)
                {
                    count++;
        ...
        public object Clone()
        {
            return new DynamicAsymmetricRoute(_first, _nextArray, _isRound);
        }
        ...
        {
            return new DynamicAsymmetricBetweenEnumerable(_nextArray, from, to, _first);
        }
        ...
        {
            if (customer == _first)
            { // if the customer to remove is the first.
                _first = _nextArray[customer];
                after = _first;
                _nextArray[customer] = -1;
        ...
        public CutResult CutAndRemove(IProblemWeights weights, double weight, int start, int length)
        {
            ...
            int[] nextArray = _nextArray.Clone() as int[];
            ...
            {
                position++; // increase the position.
                current_customer = nextArray[current_customer];
            }
            ...
            // set the next customer.
            nextArray[start_customer] = current_customer;
            weightDifference = weightDifference +
                weights.WeightMatrix[start_customer][current_customer];

            // create cut result.
            CutResult result = new CutResult();
            result.Weight = weight + weightDifference;
            result.Route = new DynamicAsymmetricRoute(this.First, nextArray, true);
        ...
        {
            int next_first = _nextArray[_first];
            //_next_array[_first] = -1;
            _nextArray[first] = next_first;
            _first = first;
        }
